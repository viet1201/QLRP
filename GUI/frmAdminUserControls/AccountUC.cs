...
namespace GUI.frmAdminUserControls
{
    public partial class AccountUC : UserControl
    {
        ...
        void UpdateAccount(string username, int accountType, string idNV)
        {
            if (AccountDAO.UpdateAccount(username, accountType, idNV))
            {
        ...
        private void btnUpdateAccount_Click(object sender, EventArgs e)
        {
            string username = txtUsername.Text;
            int accountType = (int)nudAccountType.Value;
            string idNV = cboStaffID_Account.SelectedValue.ToString();
            UpdateAccount(username, accountType, idNV);
            LoadAccountList();
        }
